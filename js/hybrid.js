...
document.getElementById("systemConstrain").onclick = function () {
    totalRequirement = parseFloat(document.getElementById('requirement').value) / sunLightHours;
...
function calculate() {
    ...
    var batteryCapacity = ((totalRequirement / (1 - parseFloat(document.getElementById('inverterLoss').value) / 100)) * 1000) / (parseFloat(document.getElementById('batteryEfficiency').value) / 100);
    var finalbatteryCapacity = (batteryCapacity * autonomy) / (dod * voltage);
    var cost = (finalArrayCapacity * 1000) * 100;
